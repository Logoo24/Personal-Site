...
**Two project JSONs, distinct purposes:** `data/featured-projects.json` holds the cards shown on the *homepage*. `data/projects.json` holds the full list shown on the *projects page*. They're independent — editing one doesn't touch the other.

**Projects page = themed cards.** Each item in `data/projects.json` renders (via `renderProject()` in `main.js`) as a site-style `.card` inside a `.proj.proj--<theme>` wrapper, in a 2-column `.proj-grid`. A theme registered on `window.PROJECT_THEMES` in `js/project-themes.js` (loaded only on `projects.html`) can fill the `<h3>` itself (e.g. with a logo; `title` is still the alt text) and supply `behind` HTML that renders under the card (`.proj-behind`, z-index 0) — that's how the ChooseAMovie popcorn bucket peeks over the card's top edge. Styles go in `css/projects.css` under `.proj--<theme>`. Unregistered themes render as a plain card. `url` makes the whole card a link — `http(s)` opens in a new tab and the corner label defaults to the domain; relative paths (tools built into this site) stay in-tab and default to "Open"; `link_label` overrides. To add a theme: add an entry in `project-themes.js`, its CSS, and the value in the Theme select in `admin/config.yml`. The `chooseamovie` theme hotlinks its logo from `https://www.chooseamovie.app/brand/logo-lockup.svg`, with a text-wordmark fallback if that fails to load.
